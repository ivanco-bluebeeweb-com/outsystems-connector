"""Connection lifecycle for OutSystems Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from outsystems_client import OutsystemsClient
...
_SECRET = "outsystems_connections"
...
@chat.function(
    "connect_outsystems",
    "Connect OutSystems account via credentials.",
    action_type="write",
    chain_callable=True,
    event="outsystems-connector.connect_outsystems",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_outsystems(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect OutSystems Connector."""
    client = OutsystemsClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "OutSystems Account",
...
@chat.function(
    "list_connections",
    "List connected OutSystems accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_outsystems",
    "Disconnect OutSystems account.",
    action_type="write",
    chain_callable=True,
    event="outsystems-connector.disconnect_outsystems",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_outsystems(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
