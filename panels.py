"""Panel UI for OutSystems Connector following UI_INTERFACE_STANDARD.md and AUTH_AND_CREDENTIALS_STANDARD.md."""
...
def _settings_button() -> ui.UINode:
    return ui.Button(
        ...
        size="sm",
        icon="settings",
        on_click=ui.Call("__panel__outsystems_settings")
    )

def _help_modal() -> ui.UINode:
    return ui.Modal(
        trigger=ui.Button("How do I set this up?", variant="ghost", size="sm"),
        title="Connecting OutSystems",
        children=[
            ui.Text(
                "1. Sign in to your OutSystems account and navigate to API/Integration or OAuth settings.\n2. Choose your preferred authentication method (OAuth SSO, API Key / Personal Token, or Client Credentials / Service Account).\n3. Authorize or enter your credentials above and click Connect.",
                variant="body"
            )
        ]
    )

@ext.panel("outsystems_sidebar", slot="left")
async def outsystems_sidebar(ctx, **kwargs) -> ui.UINode:
    return ui.Stack(
        direction="v",
        gap=3,
        align="stretch",
        children=[
            ui.Text("OutSystems", variant="heading"),
            ui.Stack(
                direction="v",
                gap=1,
                align="stretch",
                children=[
                    ui.Text("Manage your OutSystems connections and integrations.", variant="caption"),
                ]
            ),
            ui.Divider(),
            ui.Stack(
                ...
                children=[
                    ui.Button(
                        "Sign in with OutSystems (OAuth / SSO)",
                        variant="primary",
                        size="sm",
                        icon="login"
                    ),
                    ...
                    ui.Form(
                        submit_label="Connect OutSystems",
                        action=ui.Call("connect_outsystems"),
                        children=[
                            ui.Stack(
                                direction="v",
                                gap=2,
                                ...
                                children=[
                                    ...
                                    ui.Stack(
                                        ...
                                        children=[
                                            ...
                                            ui.Input(param_name="label", placeholder="e.g. Production OutSystems"),
                                        ]
                                    ),
                                    ui.Stack(
                                        direction="v",
